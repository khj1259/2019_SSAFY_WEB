...
//	EmpMgrImpl mgr=new EmpMgrImpl();   /////////////////////////  Singleton 구현 후 위의 코드로 변경할 것
	...
	private void updateRecord() {
		...
		try {
			Employee b = new Employee(Integer.parseInt(empNo), name, position, dept);
			mgr.update(b);
	...
	private void searchRecord() {
		...
		if (empNo.equals("")) {
			showList();
			lblStatus.setText("전체 직원 데이터 출력");
			return;
		}
		try {
			Employee emp = mgr.search(Integer.parseInt(empNo));
			lblStatus.setText("검색 성공");
			txtName.setText(emp.getName());
			txtPosition.setText(emp.getPosition());
			txtDept.setText(emp.getDept());
	...
	private void deleteRecord() {
		...
		String empNo = txtEmpNo.getText().trim();
		if (empNo.equals("")) {
			lblStatus.setText("직원번호 입력을 입력하세요");
			return;
		}
		try {
